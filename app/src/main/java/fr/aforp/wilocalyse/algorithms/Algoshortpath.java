...
class GFG
{
	...
	static int rowNum[] = {-1, 1, 0, 1,1,-1,-1,0};
	...
	static int BFS(int mat[][], Point src, Point dest)
	{
		...
		//Log.i("pointsss","src:"+mat[src.x][src.y]+" "+mat[src.x][src.y-1]+" "+mat[src.x][src.y+1]+" dest"+mat[dest.x][dest.y]);
		//Log.i("pointsss","src:"+mat[src.x][src.y]+" "+mat[src.x-1][src.y]+" "+mat[src.x+1][src.y]+" dest"+mat[dest.x][dest.y]);
		if (mat[src.x][src.y] == 0 || mat[dest.x][dest.y] == 0) {
			//XXXX A DEBUG XXXXXXXXX

			//quand on calcule un chemin avec le meme point de depart ou d'arrivee, on tombe ici
			return -1;
		}
